...
If a Norm is disconnected from the Hive, they will soon experience "Hive withdrawal syndrome". This starts with nervousness and sweating.
...
\subsection{Recycling: Power to X}

Thanks to green and unlimited power the Norm Hive can recycle almost anything. If it can not be repaired it is dissected into chemical elements which are stored (as simple to handle chemical components) and re-used in automated factories to build new objects. 3D printing with 10 different materials is awesome. The automation of Pioneer technology made this scale up possible.

The most commonly used component in this process is Syngas (containing carbon, hydrogen, oxygen) which is the base ingredient for all organic chemistry. Especially when creating a nutrient solution for the Ceres in vitro production of meat. All those elements can either be extracted from recycling or the atmosphere. Syngas can also used to produce polymers.
...
\subsection{Local culture}

As soon as the Hive starts working, Norms tend to build museums showcasing local culture as a large-scale community project. These can feature local food (cheese, sausage, etc.), historical figures, or other local specialities.
Without the help of experts on museums, this will turn into an amusement park spanning three levels. There will be almost no real content, but lots of entertainment to showcase the area's unique achievements.

Building one of these museums can be a task for protagonists. This involves investigating old ruins for historical artefacts.
...
%% TODO specific crimes: Spoilering episodes, den make-beliefe brechen (check wrestling) Kayfabe
