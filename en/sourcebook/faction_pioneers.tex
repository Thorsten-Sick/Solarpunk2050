...
All Pioneer communities will have at least one project in ecstatic mode. Only very large ones will be able to have 4-5 ecstatic projects running in parallel.

Examples for ecstatic/pragmatic projects are below.

\subsubsection{Scope creep}

Ecstatic Pioneer projects always suffer from scope creep. Even if the project succeeds in meeting the requirements, additional ideas for what to add always emerge. Complex and technologically challenging solutions are also favoured over simple and direct ones. In addition, anything that makes it look fancier will be added. The end result is much more complex, impressive and special than initially planned. Also: All of these features can and will break.
...
%% TODO review
The GM should introduce more glitches every time the item is used and have it eventually break down. For proper builds there are other skills than prototyping. Like crafts. But those require time and the proper tools.
...
\textbf{Rules:} When a pioneer attempts to prototype a new technology, use those steps:

\begin{itemize}
    \item {The Pioneer collects the features she wants the appliance to have as a checklist}
    \item {The Pioneer throws dice: will. The target number is the number of features}
    \item {If the target number is not reached:}
    \begin{itemize}
        \item{Make up the missing difference with adding features, one feature for each missing number. This is the feature creep happening while the Pioneer is sleepless and on drugs.}
        \item {Those features are brainstormed by the rest of the group, excluding the Pioneer. The Pioneer player can be sent to fetch Pizza if you want surprises.}
        \item {The GM picks the interesting ones}
        \item {Those features add to the intended features and are a part of the whole project}
    \end{itemize}
    \item{The Pioneer throws dice: Prototyping. Against the number of total features. If she suceeds the prototype will work without flaws.}
    \item{If the throw fails, the GM will mark a feature glitched for every missing number in this throw}
\end{itemize}

Features are either glitched from the beginning (depending on the dice throws) or soon will be. some ideas how the project will start to come apart:

\begin{itemize}
    \item Feature needs a power cycle to work
    \item Strange noises
    \item Smells like burned rubber
    \item Smoke, no need for concern
    \item Needs kicking
    \item Needs tons of lubricants
    \item Lights flicker
    \item If this feature is on, all others will be off
    \item Using this feature will also activate random other features
    \item Can not be switched off afterwards
    \item Gets really hot really fast
\end{itemize}
...
Yes, they are. They contain no animal products whatsoever. In fact, they don't contain any plant-based products either!

Just enjoy your Peking duck-flavoured ball and rest assured that no duck has been harmed!
